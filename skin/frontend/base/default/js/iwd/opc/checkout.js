...
IWD.OPC = {
    ...
    bindChangePaymentFields: function() {
        ...
        $j_opc('#payu_installment').change(function() {
            $j_opc('.agree').hide();
            IWD.OPC.Checkout.pullReview();
        });
    ...
    saveCustomerComment: function() {
        $j_opc.post(IWD.OPC.Checkout.config.baseUrl + 'onepage/json/comment', {
            "comment": $j_opc('#customer_comment').val()
        });
    },
...
IWD.OPC.Checkout = {
    ...
    pullReview: function() {
        IWD.OPC.Checkout.lockPlaceOrder();
        IWD.OPC.Checkout.xhr = $j_opc.post(IWD.OPC.Checkout.config.baseUrl + 'onepage/json/review', {
            'payment': {
                'cc_installment': jQuery('#payu_installment').val()
            }
        }, function(response) {
            IWD.OPC.Checkout.xhr = null;
            IWD.OPC.Checkout.hideLoader();
    ...
    save: function() {
        ...
        IWD.OPC.Checkout.ajaxProgress = setTimeout(function() {
            ...
            if ($j_opc('input[name="billing[use_for_shipping]"]').is(':checked')) {
                console.log('use for shipping clicked');
                IWD.OPC.Checkout.showLoader();
            } else {
