...
/*        $page->drawText(
        ...
        $page->setFillColor(new Zend_Pdf_Color_GrayScale(1));
        //$page->drawRectangle(25, ($top - 25), 349, $top - 33 - $addressesHeight);
        //$page->drawRectangle(399, ($top - 25), 723, $top - 33 - $addressesHeight);
        // /$page->drawRectangle(773, ($top - 25), 1097, $top - 33 - $addressesHeight);
        $page->setFillColor(new Zend_Pdf_Color_GrayScale(0));
        ...
        foreach ($billingAddress as $value){
            if ($value !== '') {
                ...
                foreach ($text as $part) {
                    ...
                    $page->drawText(strip_tags(ltrim($part)), (35+374), $this->y, 'UTF-8');
                    $page->drawText(strip_tags(ltrim($part)), (35+748), $this->y, 'UTF-8');
                    $this->y -= 15;
                }
            }
        }
        ...
        $page->drawText("İRS. TARİH ".date("d.m.Y"), 245, $this->y, 'UTF-8');
        $page->drawText("İRS. TARİH ".date("d.m.Y"), (245+374), $this->y, 'UTF-8');
        $page->drawText("İRS. TARİH ".date("d.m.Y"), (245+748), $this->y, 'UTF-8');
        $this->y -= 15;
        $page->drawText("FTR. TARİH ".date("d.m.Y"), 245, $this->y, 'UTF-8');
        $page->drawText("FTR. TARİH ".date("d.m.Y"), (245+374), $this->y, 'UTF-8');
        $page->drawText("FTR. TARİH ".date("d.m.Y"), (245+748), $this->y, 'UTF-8');
...
/*
    ...
    protected function insertTotals($page, $source){
        ...
        error_log(floatval($ss)."kkl");

        foreach ($totals as $total__2) {
            $total__2->setOrder($order)
                ->setSource($source);

            if ($total__2->canDisplay()) {
                $total__2->setFontSize(10);
                foreach ($total__2->getTotalsForDisplay() as $totalData2) {
                    if ($totalData2['label'] == "Kargolama ve Teslim Alma:") {
                         $shipping_amount = $order->getShippingAmount();

                        $kargoParasi = $shipping_amount;
                    }
                }
            }
        }

                    $kargoParasi = round($order->getShippingAmount() / 1.18, 2);
        ...
        foreach ($totals as $total) {
            ...
            if ($total->canDisplay()) {
                ...
                foreach ($total->getTotalsForDisplay() as $totalData) {
                    ...
                    if ($totalData['label'] == "Ara Toplam:"):
                        $ss = explode("\xc2\xa0", $totalData['amount']);
                    $subtotal_ = str_replace(",", ".", $ss[0]);
                    $subtotal_ = round($subtotal_/1.18,2);
              
                    
                        $kk = $subtotal_ + $kargoParasi;
                        //$newkk = number_format(($kk*82/100),2,",",",") . " TL";
			$newkk = number_format($kk,2,",",",") . " TL";
                    ...
                    $newkk = number_format((str_replace(',', '.', $newkk)*0.18),2,",",",") . " TL";
                    $lineBlock['lines'][] = array(
                        array(
                            'text'      => "KDV %18",
                            'feed'      => 275,
                    ...
                    elseif ($totalData['label'] == "Kargolama ve Teslim Alma:"):
                        
                    elseif ($totalData['label'] == "Teslimat:"):
        ...
        $this->y += 45;
        $page = $this->drawLineBlocks($page, array($lineBlock2));
        $this->y += 45;
        $page = $this->drawLineBlocks($page, array($lineBlock3));
/*
        $this->y -= 90;
	$page = $this->drawLineBlocks($page, array($lineBlock));
        $this->y += 75;
...
//        exit;
    ...
    protected function insertTotals2($page, $source){
        $order = $source->getOrder();
//        print_r($order);exit;
        $totals = $this->_getTotalsList($source);
        $lineBlock = array(
        ...
        error_log("imp".$this->yImp);
        foreach ($totals as $total) {
            $total->setOrder($order)
                ->setSource($source);

        /* freeshipping_freeshipping => ücretsiz
             * phoenix_cashondelivery => kapıda ödeme
             * flatrate_flatrate =>ücretli
             * 
             */
            ...
            $shipping_amount = number_format(round($order->getShippingAmount() / 1.18, 2), 2);
     ...
     $star_line_label = "";
            ...
            if ($shipping_method == "freeshipping_freeshipping") {
                $shipment_label = "ÜCRETSİZ KARGO ";
                if ($payment_method_code == "phoenix_cashondelivery") {
                    $star_line_label = "*******************KAPIDA ÖDEME";
                }
            } else if ($shipping_method == "flatrate_flatrate") {
                $shipment_label = "KARGO BEDELİ ";
                if ($payment_method_code == "phoenix_cashondelivery") {
                    $star_line_label = "*******************KAPIDA ÖDEME";
                }else{
//                    continue;
                }
            } else if ($shipping_method == "zerobuffer_mycarrier_standand") {
                $shipment_label = "KURYE BEDELİ ";
                $star_line_label = "*******************KURYE İLE TESLİMAT";
            } else if ($payment_method_code == "phoenix_cashondelivery") {
                $shipment_label = "KAPIDA ÖDEME BEDELİ ";
                $star_line_label = "*******************KAPIDA ÖDEME";
            }



            if ($total->canDisplay() && $displayed_shipment_method!=true) {
                $total->setFontSize(10);
                $displayed_shipment_method = true;
                    error_log("aaa".$totalData['label']);
                    if($payment_method_code !="freeshipping_freeshipping" ):
                    $lineBlock['lines'][] = array(
                        array(
                            'text'      => $shipment_label,
                            'feed'      => 35
...
//                }
     ...
     * @return Zend_Pdf_Page
     */
    public function drawLineBlocks(Zend_Pdf_Page $page, array $draw, array $pageSettings = array(), $yImp = false)
    {
