...
abstract class Mage_Sales_Model_Order_Pdf_Items_Abstract extends Mage_Core_Model_Abstract
{
    ...
    public function getItemPricesForDisplay()
    {
        ...
        if (Mage::helper('tax')->displaySalesBothPrices()) {
            ...
            $prices = array(array(
             // 'price' => $order->formatPriceTxt($item->getPrice()*82/100),
                'price' => $order->formatPriceTxt($item->getPrice()/1.18),
	//	'subtotal' => $order->formatPriceTxt($item->getRowTotal()*82/100),
                'subtotal' => $order->formatPriceTxt($item->getRowTotal()/1.18),
            ));
        }
        return $prices;
    }
